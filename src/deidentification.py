...
substring_whitelist_set = ['right','left','mid','lateral','apex','base','gleason']
custom_whitelist = ['inked', 'mid', 'gross', 'prostate', 'grade', 'primary', 'secondary', 'medial', 'lateral', 'seminal', 'vesicle', 'midgland', 'apical', 'left', 'right', 'base', 'acinar', 'neoplastic', 'carcinoma', 'perineural', 'benign', 'malignant', 'malignancy', 'core', 'apex', 'fragments', 'adenocarcinoma', 'anterior', 'gleason', 'final diagnosis', 'diagnosis', 'Seminal', 'Vesicle', 'Gleason']
...
def namecheck(word_output, name_set, screened_words, safe, nlp):
    # check if the word is in the name list
    if (word_output.lower() not in custom_whitelist) and not any(
            [phrase in word_output.lower() for phrase in substring_whitelist_set]):
...
def filter_task(note, nlp):
    with open('whitelist.pkl', "rb") as fin:
        whitelist_dict = pickle.load(fin)
    safe = True
    screened_words = []
    name_set = set()
    phi_reduced = ''
            ...
            if word_output not in string.punctuation:
                ...
                try:
                    ...
                    if (((word[1] == 'NN' or word[1] == 'NNP') or
                        ...
                        if (word_check.lower() not in whitelist_dict) and (word_check.lower() not in custom_whitelist) and not any([phrase in word_check.lower() for phrase in substring_whitelist_set]):
                            screened_words.append(word_output)
                            word_output = "PHI"
                            safe = False
                        else:
                            ...
                            if ((word_output.istitle() or word_output.isupper()) and
                                ...
                                word_output, name_set, screened_words, safe = namecheck(word_output, name_set, screened_words, safe, nlp)
                    ...
                    # check day/year according to the month name
                        ...
                        word_output, name_set, screened_words, safe = namecheck(word_output,
                                                                                name_set, screened_words, safe, nlp)
    ...
    return phi_reduced.replace("||| BREAK |||", "\n").replace(" . ", " . \n")
